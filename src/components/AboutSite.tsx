  ...
  return (
    ...
    >
      <div onClick={onPress} className={styles.modalContainer(isVisible, isOpen)}>
        ...
        <div className={styles.imageContainer(isVisible && isOpen)}>{image}</div>
        <p className={styles.explanation(isVisible && isOpen)}>{explanation}</p>
        <div className={styles.questionMarkIcon(isVisible && !isOpen)}>
          <QuestionMarkIcon />
        </div>
      </div>
    </div>
...
const styles = {
  ...
  modalContainer: (isVisible: boolean, isOpen: boolean) => css`
    ...
    border-radius: 32px;
    box-shadow: ${isVisible ? '10px 10px 20px #d0d3d7, -8px -8px 16px #ffffff' : 'none'};
  ...
  closeButton: (isVisible: boolean) => css`
    ...
    top: 16px;
    right: 16px;
    transition: all 0.3s ease-in-out ${isVisible ? '0.4s' : '0s'};
    border: none;
    box-shadow: ${isVisible ? '10px 10px 20px #d0d3d7' : 'none'}, ${isVisible ? '-8px -8px 16px #ffffff' : 'none'};
  ...
  questionMarkIcon: (isVisible: boolean) => css`
    transition: all 0.2s ease-in-out ${isVisible ? '1s' : '0s'};
    opacity: ${isVisible ? 1 : 0};
    position: absolute;
    width: 12px;
    height: 12px;
  ...
  imageContainer: (isVisible: boolean) => css`
    transition: all 0.3s ease-in-out ${isVisible ? '0.6s' : '0s'};
    box-shadow: inset 8px 8px 16px #d0d3d7, inset -8px -8px 16px #ffffff;
    width: 100%;
    height: 200px;
    border-radius: 16px;
    margin: 24px 0px;
